...
interface DoctorProfile {
  id: string;
  full_name: string;
  email: string; // Assuming email is in the main profile
  role: string;
  photoUrl?: string; // Assuming photoUrl might be here or in doctor_profiles
  doctor_profiles: {
    ...
    isAvailableToday?: boolean; // Added availability
    patientCount?: number; // Added stats
    reviews?: number; // Added stats
    isVerified?: boolean; // Added verification
    availability_schedule?: unknown; // Use unknown or a specific type
    profile_id?: string; // Foreign key
    // ... other fields from 'doctor_profiles' table ...
  };
}

// import Button from "../common/Button"; // Assuming Button is in common components

interface DoctorCardProps {
  doctor: DoctorProfile; // Changed from Doctor to DoctorProfile
}
          ...
          <div className="flex items-center gap-1">
            {[...Array(5)].map((_, i) => (
              <Star
                ...
                size={14}
                fill={
                  i < Math.floor(doctor.doctor_profiles.rating || 0)
                    ? "#facc15"
                    : "none"
                }
                color={
      ...
      <div className="p-5 pt-14 flex-1 flex flex-col">
        ...
        <div className="space-y-3 mb-6">
          <div className="flex items-center text-sm text-neutral-600 dark:text-neutral-400">
            ...
            <span>
              {doctor.doctor_profiles.clinic_address || "Address not available"}
            </span>{" "}
            {/* Added fallback */}
          </div>
          ...
          <div className="flex items-center text-sm text-neutral-600 dark:text-neutral-400">
            ...
            {/* Check if availability_schedule exists before trying to get keys */}
            <span>
              Available:{" "}
              {doctor.doctor_profiles.availability_schedule
                ? Object.keys(
                    doctor.doctor_profiles.availability_schedule
                  ).join(", ")
                : "Check profile"}
        ...
        <div className="flex justify-between mb-6 px-1">
          <div className="text-center">
            <p className="text-lg font-bold text-blue-600 dark:text-blue-400">
              {doctor.doctor_profiles.experience_years || 0}+
            </p>{" "}
            {/* Added fallback */}
          ...
          <div className="text-center">
            <p className="text-lg font-bold text-green-600 dark:text-green-400">
              {doctor.doctor_profiles.patientCount || 0}+
            </p>
            <p className="text-xs text-neutral-600 dark:text-neutral-400">
              Patients
            </p>
          ...
          <div className="text-center">
            <p className="text-lg font-bold text-purple-600 dark:text-purple-400">
              {doctor.doctor_profiles.reviews || 0}
            </p>
            <p className="text-xs text-neutral-600 dark:text-neutral-400">
              Reviews
            </p>
