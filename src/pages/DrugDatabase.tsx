...
import DrugCardSkeleton from "../components/drugs/DrugCardSkeleton";
    ...
    a: [
      {
        ...
        dosage:
          "Adults: 325-650 mg every 4-6 hours as needed, not to exceed 4000 mg per day.",
        sideEffects: [
      ...
      },
      {
        ...
        dosage:
          "Initially 10-20 mg once daily. Maintenance: 10-80 mg once daily, preferably in the evening.",
        sideEffects: [
    ...
    b: [
      {
        ...
        dosage:
          "Depression: Initially 150 mg once daily for 3 days, then 150 mg twice daily. Smoking cessation: 150 mg once daily for 3 days, then 150 mg twice daily for 7-12 weeks.",
        sideEffects: [
          "Dry mouth",
          "Headache",
          "Insomnia",
      ...
      },
      {
        ...
        dosage:
          "Adults and children over 10 years: 100-200 mg three times daily as needed, not to exceed 600 mg per day.",
        sideEffects: [
          "Drowsiness",
          "Headache",
          "Constipation",
    ...
    l: [
      {
        ...
        dosage:
          "Hypertension: Initially 10 mg once daily, maintenance 20-40 mg once daily. Heart failure: Initially 5 mg once daily, target 20-40 mg once daily.",
        sideEffects: [
          "Dry cough",
          "Dizziness",
          "Headache",
...
const searchDrugs = async (query: string): Promise<Drug[]> => {
  ...
  try {
    const results = await Promise.all(
      letters.map((letter) => fetchDrugs(letter))
    );
...
export default function DrugDatabase() {
  ...
  const loadDrugs = async (letter: string) => {
    setIsLoading(true);
    setErrorMessage(null);
    setActiveFilters({ category: [], usageType: [], prescriptionStatus: [] });
    try {
  ...
  const resetSearch = () => {
    ...
    loadDrugs(activeLetter); // Reload current letter's data
  };
      ...
      <div className="relative container mx-auto px-4 py-16 md:py-24">
        ...
        >
          ...
          <p className="text-xl text-gray-600 dark:text-gray-300 max-w-3xl mx-auto leading-relaxed">
            Detailed information on medications including uses, dosage
            instructions, side effects, interactions, and important warnings.
          </p>
        </motion.div>
                ...
                >
                  ...
                  {hasActiveFilters
                    ? `Filters (${
                        ...
                        activeFilters.usageType.length +
                        activeFilters.prescriptionStatus.length
                      })`
                    : "Filters"}
                </motion.button>
              </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {filterOptions.prescriptionStatus.map((status) => (
                        <motion.button
                          key={status}
                          onClick={() =>
                            toggleFilter("prescriptionStatus", status)
                          }
                          whileHover={{ scale: 1.05 }}
                          whileTap={{ scale: 0.95 }}
                          className={`px-3 py-1 rounded-full text-sm font-medium transition-all duration-200 ${
